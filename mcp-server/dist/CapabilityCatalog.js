...
function inferCapabilities(mod) {
...
export class CapabilityCatalogManager {
    ...
    buildCatalogFromManifest(manifest) {
        ...
        const capabilityToModules = Object.fromEntries(CAPABILITIES.map((cap) => [cap, []]));
    ...
    buildFallbackCatalogFromLegacy(legacy) {
    ...
    async refreshFromRemoteManifest() {
    ...
    async getCatalog() {
        ...
        if (await fs.pathExists(this.catalogCachePath)) {
            return (await fs.readJson(this.catalogCachePath));
        }
        if (await fs.pathExists(this.manifestCachePath)) {
            const manifest = (await fs.readJson(this.manifestCachePath));
        ...
        const emptyCatalog = {
            ...
            modules: [],
            capabilityToModules: Object.fromEntries(CAPABILITIES.map((cap) => [cap, []])),
        };
        await fs.writeJson(this.catalogCachePath, emptyCatalog, { spaces: 2 });
        return emptyCatalog;
    }
    async lookupByCapabilities(requestedCapabilities) {
        const catalog = await this.getCatalog();
        const capabilityMatches = Object.fromEntries(requestedCapabilities.map((cap) => [cap, catalog.capabilityToModules[cap] ?? []]));
