...
export class DatapackContractValidator {
  ...
  private normalizeFunctionPath(namespace: string, rel: string): string {
  ...
  async validate(): Promise<ContractValidationResult> {
    ...
    const pack = await this.readJsonIfExists<{ pack?: Record<string, unknown> }>(packPath);
    if (!pack?.pack || typeof pack.pack.pack_format !== "number") {
      errors.push("pack.mcmeta invalide: champ pack.pack_format manquant.");
    } else {
      const packFormat = pack.pack.pack_format as number;
      ...
      if (packFormat <= 81) {
        if (!Array.isArray(rawSupported) || rawSupported.length !== 2) {
          errors.push(
            "pack.mcmeta invalide: pour pack_format <= 81, supported_formats doit être [min,max] (ex: [81,81])."
          );
        } else {
          const supportedFormats = rawSupported.map((x) => Number(x));
          details.supportedFormats = supportedFormats;
          if (
            !Number.isFinite(supportedFormats[0]) ||
            ...
            errors.push(
              `pack.mcmeta incohérent: pack_format=${packFormat} hors plage supported_formats=[${supportedFormats.join(",")}].`
            );
          }
        }
      }
    ...
    if (!tickTagPath) errors.push("Tag tick manquant: data/minecraft/tags/function/tick.json");
    if (!loadTagPath) errors.push("Tag load manquant: data/minecraft/tags/function/load.json");

    const tickTag = tickTagPath
      ? await this.readJsonIfExists<{ values?: string[] }>(path.join(this.rootDir, tickTagPath))
      : null;
    const loadTag = loadTagPath
      ? await this.readJsonIfExists<{ values?: string[] }>(path.join(this.rootDir, loadTagPath))
      : null;
    ...
    for (const advPath of advancements) {
      try {
        const adv = (await fs.readJson(path.join(this.rootDir, advPath))) as Record<string, unknown>;
        const criteria = adv.criteria as Record<string, unknown> | undefined;
        ...
        for (const [criterionName, criterionValue] of Object.entries(criteria)) {
          const criterion = criterionValue as Record<string, unknown>;
          if (!criterion || typeof criterion.trigger !== "string") {
            errors.push(
              `Advancement invalide (${advPath}): trigger manquant pour le critère '${criterionName}'.`
            );
          }
        }
      } catch (e: any) {
    ...
    if (loadTag?.values?.length) {
      ...
      for (const ref of loadTag.values) {
        ...
        if (hasScoreboardBootstrap) break;
      }
      if (!hasScoreboardBootstrap) {
        warnings.push(
          "Aucun bootstrap scoreboard détecté dans les fonctions load taguées. Vérifier l'initialisation runtime."
        );
      }
    }
