...
export class BONEEngine {
  private static TEMPLATE_URL = "https://github.com/TheophileBaudouin/template-BONE-MSD.git";
  private static ZIP_URL = "https://github.com/TheophileBaudouin/template-BONE-MSD/archive/refs/heads/main.zip";
   ...
   */
  async ingestTemplate(): Promise<string> {
...
`;
  ...
  private async listFunctionsRecursive(dir: string, base: string = ""): Promise<string[]> {
  ...
  private async validateMCDoc(tmpDir: string) {
   ...
   */
  async initTemplate(): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
      ...
      checkGit.on("close", (code) => {
        if (code !== 0) return reject(new Error("Git n'est pas accessible."));
   ...
   */
  async verifyAssets(): Promise<{ missing: string[]; found: string[] }> {
...
`;
  ...
  async updateAssetsTodo(items: Array<{ name: string; type: 'block' | 'item'; namespace: string }>) {
    const todoPath = path.join(this.rootDir, "ASSETS_TODO.md");
