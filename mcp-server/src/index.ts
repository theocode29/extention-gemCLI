...
import { CAPABILITIES, CapabilityCatalogManager, type Capability } from "./CapabilityCatalog.js";
...
function inferCapabilitiesFromTask(task: string): Capability[] {
  ...
  try {
    switch (name) {
      ...
      case "agent_research_validate": {
        ...
        const missingKinds = ["library_doc", "minecraft_runtime_doc", "internal_doc"].filter(
          (k) => !byKind.has(k as "library_doc" | "minecraft_runtime_doc" | "internal_doc")
        );
        ...
        const incoherentCapabilities = evidenceCaps.filter((cap) => {
          const knownModules = (catalog.capabilityToModules as Record<string, string[]>)[cap] ?? [];
          if (knownModules.length === 0) return false;
          if (selectedSet.size === 0) return true;
          return !knownModules.some((mod) => selectedSet.has(mod));
        });
        const hasCapabilityCoherence = incoherentCapabilities.length === 0;
        const hasRejectedWithRationale =
          mission.research.rejectedModules.every((mod) =>
            mission.research.constraints.some((c) => {
      ...
      case "agent_library_route": {
        ...
        const hasSpecializedModules = requestedCapabilities.some(
          (cap) => (lookup.capabilityMatches[cap] ?? []).length > 0
        );
        ...
        let route: "bone" | "bookshelf" | "vanilla" | "hybrid" = "vanilla";
        ...
        let fallbackRoute: "bone" | "bookshelf" | "vanilla" | "hybrid" = "vanilla";
      ...
      case "datapack_physics_profile_apply": {
        ...
        const mission = await missionManager.setCheckpoint(
          "physics_profile",
          `Applied ${parsed.profile_name} -> ${relPath}`
        );
        return {
