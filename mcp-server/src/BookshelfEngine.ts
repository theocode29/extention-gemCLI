...
export class BookshelfEngine {
  private static ZIP_URL = "https://github.com/mcbookshelf/bookshelf/archive/refs/heads/master.zip";
  private static BONE_URL = "https://github.com/BONE-MSD/core/archive/refs/heads/main.zip";
  ...
  async ingest(): Promise<string> {
    ...
    const knowledge: BookshelfKnowledge = {
      version: "latest",
  ...
  async addDependency(libName: string, targetModules?: string[]): Promise<string> {
      const docsDir = path.join(this.rootDir, ".docs");
      ...
      const resolve = (m: string) => {
          if (processed.has(m) || !knowledge.modules.includes(m)) return;
          processed.add(m);
          toInstall.add(m);
