...
export type InitProfile = "minimal" | "worldgen" | "tests" | "full";
...
export interface ResolvedVersionSpec {
  ...
  supportsSingularFolders: boolean;
  supportsDataDrivenEnchantments: boolean;
  supportsGameTests: boolean;
  strictJsonParsing: boolean;
  usesMinMaxFormatModel: boolean;
}
...
export interface ProjectInitResult {
  ...
  profile: InitProfile;
  namespace: string;
  resolvedVersion: ResolvedVersionSpec;
  createdPaths: string[];
  warnings: string[];
}
...
function normalizeVersion(v: ParsedVersion): string {
  return `${v.major}.${v.minor}.${v.patch}`;
}

function ensureSupportedVersion(v: ParsedVersion): void {
...
function resolvePackFormat(v: ParsedVersion): number {
  ...
  if (compareVersions(v, v1217) >= 0) return 81;
  if (compareVersions(v, v1216) >= 0) return 80;
  if (compareVersions(v, v1215) >= 0) return 71;
  if (compareVersions(v, v1210) >= 0) return 48;
  if (compareVersions(v, v1205) >= 0) return 41;
...
function validateNamespace(namespace: string): string {
  ...
  if (!/^[a-z0-9_.-]+$/.test(normalized)) {
    throw new Error(
      "Namespace invalide. Caractères autorisés: lettres minuscules, chiffres, underscore (_), point (.), tiret (-)."
    );
  }
...
export class DatapackInitEngine {
  ...
  public resolveVersionSpec(version: string): ResolvedVersionSpec {
  ...
  public async initProject(input: ProjectInitInput): Promise<ProjectInitResult> {
    const profile: InitProfile = input.profile ?? "minimal";
  ...
  private async writePackMcmeta(spec: ResolvedVersionSpec, createdPaths: string[]): Promise<void> {
  ...
  private buildFolderPlan(
    spec: ResolvedVersionSpec,
    namespace: string,
    profile: InitProfile,
    warnings: string[]
  ): string[] {
