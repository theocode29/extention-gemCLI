...
export class BONESemanticLinter {
   ...
   */
  async lint(targetPath: string): Promise<LinterResult> {
  ...
  private async lintRecursiveCycles(dataDir: string, errors: string[]) {
  ...
  private async lintDirectory(dir: string, errors: string[]) {
  ...
  private async lintFile(filePath: string, errors: string[]) {
